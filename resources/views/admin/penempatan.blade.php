...
@section('content')
   ...
   {{-- Judul & breadcrumb dihapus — topbar (layouts.admin) sudah
    ...
    {{-- ============================================================
         3. TAB SWITCHER — Penempatan Resmi | Pengajuan Masuk
         ============================================================ --}}
    <div class="penempatan-tabs">
        <button type="button" class="penempatan-tab-btn active" data-tab="tab-penempatan">
            <i class="bi bi-diagram-3-fill"></i> Penempatan Resmi
        </button>
        <button type="button" class="penempatan-tab-btn" data-tab="tab-pengajuan">
            <i class="bi bi-send-fill"></i> Pengajuan Masuk
            @if ($pengajuanMenunggu > 0)
                <span class="tab-badge">{{ $pengajuanMenunggu }}</span>
            @endif
        </button>
    </div>

    {{-- ============================================================
         TAB 1 — PENEMPATAN RESMI
         ============================================================ --}}
    <div id="tab-penempatan" class="tab-panel active">
            ...
            <tbody>
                @forelse ($penempatanList as $item)
                    <tr>
                        ...
                        <td>
                            @if ($item->status_pengesahan === 'disahkan')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    /* =========================================================
       ...
       ========================================================= */
    const tabBtns   = document.querySelectorAll('.penempatan-tab-btn');
    ...
    // Cek parameter URL ?tab=pengajuan atau ?cari_pengajuan atau ?status_pengajuan
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('tab') === 'pengajuan' || urlParams.has('cari_pengajuan') || urlParams.has('status_pengajuan') || urlParams.has('halaman_pengajuan')) {
        switchTab('tab-pengajuan');
    }
    ...
    document.getElementById('btnSetujuiConfirm').addEventListener('click', async function () {
        ...
        try {
            const res  = await fetch(`/admin/pengajuan/${pendingSetujuiId}/setujui`, {
                method:  'PATCH',
            ...
            if (res.ok) {
                ...
                window.location.href = baseUrl + '?tab=pengajuan';
            } else {
                alert(data.message ?? 'Gagal menyetujui pengajuan.');
            }
        } catch (e) {
    ...
    document.getElementById('btnTolakConfirm').addEventListener('click', async function () {
        ...
        try {
            ...
            const res  = await fetch(`/admin/pengajuan/${pendingTolakId}/tolak`, {
                method:  'POST',
                headers: { 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' },
                body,
            });
